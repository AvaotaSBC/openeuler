...
default_param() {
    repo_file="https://gitee.com/src-openeuler/openEuler-repos/raw/openEuler-22.03-LTS/generic.repo"
    tmp_dir=${workdir}/tmp
    workdir=$(pwd)/build
...
EOF
    ...
    cp -r $nonfree_bin_dir/firmware/* ${rootfs_dir}/usr/lib/firmware/
...
if [ -d rootfs ]; then
    if [[ -f $workdir/rootfs.img && $(cat $workdir/.done | grep rootfs) == "rootfs" ]];then
        ...
        if [[ ${last_branch} != ${branch} || ${last_repo_file} != ${repo_file} || ${last_spec_param} != ${spec_param} ]]; then
            rm -rf rootfs
            build_rootfs
            mk_rootfsimg
        fi
